...
def black_scholes(contract: OptionContract, market: MarketState, return_greeks: bool = False) -> Union[float, np.ndarray, Tuple[Union[float, np.ndarray], Dict[str, Union[float, np.ndarray]]]]:
    ...
    if np.any(mask_calc):
        ...
        if contract.option_type == OptionType.CALL:
            ...
            if return_greeks:
                delta[mask_calc] = df_q * norm.cdf(d1)
                gamma[mask_calc] = df_q * norm.pdf(d1) / (S_c * sig_c * np.sqrt(T_c))
                vega[mask_calc] = S_c * df_q * norm.pdf(d1) * np.sqrt(T_c)
                theta[mask_calc] = -df_q * S_c * norm.pdf(d1) * sig_c / (2 * np.sqrt(T_c)) - r_c * K_c * df_r * norm.cdf(d2) + q_c * S_c * df_q * norm.cdf(d1)
                rho[mask_calc] = K_c * T_c * df_r * norm.cdf(d2)
        else:
