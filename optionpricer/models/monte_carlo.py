...
def price_exotic(contract: OptionContract, market: MarketState, N_paths: int = 16_384, N_steps: int = 252, seed: int = None) -> Union[float, np.ndarray]:
...
def monte_carlo_prices(contract: OptionContract, market: MarketState, N: int = 32_768, seed: int = None) -> Union[float, np.ndarray]:
    return price_exotic(contract, market, N_paths=N, N_steps=1, seed=seed)
